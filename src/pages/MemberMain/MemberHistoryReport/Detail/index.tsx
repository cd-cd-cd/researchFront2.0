...
import { addreportcomment, postAvatar } from '../../../../api/Member'
...
export default function Detail({ detail, detailReturn }: Props) {
  ...
  const returnComments = async () => {
    if (comments.content === '<p><br></p>' || !comments.content) {
      ...
      if (roleGroup) {
        const res = await addreportcomment(
          detail.id,
          comments.content,
          roleGroup
        )
    ...
    <div className={style.detail}>
      <div className={style.main} id="report">
        <div className={style.submit}>
          <Button onClick={detailReturn}>返回</Button>
        </div>
        ...
        <div className={style.partOne}>
          ...
          <div className={style.border}>
            <div
              dangerouslySetInnerHTML={{ __html: detail?.weekProgress.content }}
            ></div>
            {detail?.weekProgress.attach.map((item) => (
        ...
        <div className={style.partOne}>
          ...
          <div className={style.border}>
            <div
              dangerouslySetInnerHTML={{ __html: detail?.weekPlan.content }}
            ></div>
            {detail?.weekPlan.attach.map((item) => (
              <div key={item.url} className={style.urlBox}>
                <a href={item.url} className={style.url}>
                  {item.fileName}
                </a>
        ...
        {detail?.teamWorks.length ? (
          <div className={style.partOne}>
            ...
            <div>
              {detail?.teamWorks.map((item, index) => (
                <div key={item.id} className={style.border}>
                  <div>
                    <div className={style.Teamworktitle}>
                      {((index + 1).toString() as string) +
                        '. ' +
                        (item.title as string)}
                    </div>
                    <div className={style.type}>
                      类型：{' '}
                      {item.type ? renderType(item.type as IWorkType) : ''}
                    </div>
                    <div className={style.time}>
                      服务时长：{item.duration} 个半天
                    </div>
                  </div>
                  <div
                    style={{ margin: '2px' }}
        ...
        <div className={style.commentDiv}>
          <div className={style.btn_box}>
            <Button
              type={commentType === 0 ? 'primary' : 'default'}
              onClick={() => setCommentType(0)}
            >
              管理员
            </Button>
            <Button
              type={commentType === 1 ? 'primary' : 'default'}
              onClick={() => setCommentType(1)}
            >
              组长
            </Button>
          </div>
          <div className={style.infos_div}>
            {commentType === 0
              ? infos?.adminGroupComments.map((item) => (
                  <Comment
                    key={item.id}
                    author={<a>{item.userInfo.studentNo}</a>}
                    // avatar={
                    //   <Avatar
                    //     src={item.userInfo.photo}
                    ...
                    content={
                      <div>
                        ...
                        ></div>
                        {item.isMyself === 1 &&
                        !item.content.includes('该评论已于') ? (
                          <div className={style.box_return}>
                            <div
                              className={style.returnComment}
                              onClick={() => delComments(item.id)}
                            >
                              撤回
                            </div>
                          </div>
            ...
            {commentType === 1
              ? infos?.leaderGroupComments.map((item) => (
                  <Comment
                    ...
                    author={
                      <a>
                        {item.userInfo.username + '-' + item.userInfo.studentNo}
                      </a>
                    }
        ...
        <div className={style.editorMain}>
          <div style={{ border: '1px solid #ccc', zIndex: 100 }}>
            ...
            <Editor
              ...
              onCreated={setEditor}
              onChange={(editor) =>
                setComments({ ...comments, content: editor.getHtml() })
              }
              mode="default"
