...
export default function MSBReport() {
        ...
        res.data.reduce((pre: IOption[], cur) => {
          pre.push({
            label:
              ...
              '-' +
              cur.user.studentNo +
              (cur.notReadCnt !== 0 ? ' ' + cur.notReadCnt + '条未读' : ''),
  ...
  const clickSearch = async (firstClick: boolean) => {
    if (checkNoAndRange() && value && value[0] && value[1]) {
      ...
      if (checkTime(startTimeInfo, endTimeInfo)) {
        const res = await getuserteamwork(
          current,
          10,
          chooseNO!,
          startTimeInfo,
  ...
  const reportSearch = async (firstClick: boolean) => {
    if (checkNoAndRange() && value && value[0] && value[1]) {
      ...
      const endTimeInfo = returnYMW(value[1], time[1])
      if (checkTime(startTimeInfo, endTimeInfo)) {
        const res = await getusertimereport(
          current,
          10,
          chooseNO!,
          startTimeInfo,
  ...
  const clickDetailMoal = (record: ITeamWorkItem) => {
    ...
    setDetailInfo({
      ...
      attach: JSON.parse(record.attach)
    })
  }
  ...
  return (
    <div className={style.back}>
      ...
      {searchType === 0 || searchType === 1 ? (
        ...
        >
          ...
          <Column
            ...
            key="adminStatus"
            render={(value: 0 | 1 | 2) => renderastatus(value)}
          ></Column>
          <Column
            title="周报状态(组长)"
          ...
          <Column
            ...
            render={(_: any, record: IWeeklyReports) => (
              <div className={style.a_box}>
                <a onClick={() => clickView(record)}>点击查看</a>
              </div>
            )}
          />
        </Table>
        ...
        >
          ...
          <Column
            ...
            dataIndex="type"
            key="type"
            render={(value: IWorkType) => renderType(value)}
          />
          <Column
            title="团队贡献时长"
            dataIndex="duration"
            key="duration"
            render={(value: string) => value + '（半天）'}
          ></Column>
          <Column
            title="创建时间"
            dataIndex="createTime"
          ...
          <Column
            title="详情"
            render={(_: any, record: ITeamWorkItem) => (
              <a onClick={() => clickDetailMoal(record)}>点击查看详情</a>
            )}
          />
        </Table>
      )}
      {detail ? (
        <Detail
          getSource={() => getSource(false)}
          reportSearch={() => reportSearch(false)}
          searchType={searchType}
          getUserList={getUserList}
          detail={detail}
          detailReturn={() => setDetail(undefined)}
        ></Detail>
      ) : null}
      {
        <Modal
