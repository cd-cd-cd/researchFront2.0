...
import MDeviceApply from './MDeviceApply'
...
export default function MDevice() {
  // 添加设备form
  const [form] = useForm()
  const [modifyForm] = useForm()
  const [useDeviceForm] = useForm()
  // 添加设备Modal
  ...
  // 控制设备申请
  const [isUseDevice, setIsUseDevice] = useState(false)
  // 保存设备申请信息
  const [useDevice, setUseDevice] = useState<IDevice>()
  // 控制drawer
  ...
  const deviceApply = async (values: any) => {
    if (useDevice?.id) {
      ...
      const res = await managerUseDevice(useDevice.id, date)
      if (res?.code === 200) {
      ...
      message.info('请返回重写选择设备')
    }
  }

  // 点击使用设备按钮
  const useModalClick = (record: IDevice) => {
    setIsUseDevice(true)
    setUseDevice(record)
  }

  // 关闭使用设备Modal
  const closeUseDevice = () => {
    useDeviceForm.resetFields()
    setIsUseDevice(false)
  }
  ...
  const confirm = async (id: string) => {
    const res = await recoverapplyrecord(id)
    if (res?.code === 200) {
      ...
      <div className={style.height2}>
        ...
        >
          ...
          <Column title="领用人" dataIndex="recipient" key="recipient"
            render={(value: IRecipient | null, record: ISource) => value
              ? <Popconfirm
                title="你确定要提前收回该设备吗"
                onConfirm={() => confirm(record.id)}
                okText="是"
                cancelText="否"
              ><a>{value.studentNo + value.username}</a>
              </Popconfirm>
              : '无'
            }
          />
          <Column title='操作'
            render={(_: any, record: IDevice) =>
              <div className={style.a_box}>
                ...
                {
                  record.status === 1 ? <Button size='small' onClick={() => useModalClick(record)}>使用该设备</Button> : null
                }
                {
        ...
        >
          ...
          <Form.Item>
            <div className={style.btn_box}>
              ...
              <Button htmlType="button" onClick={() => closeUseDevice()}>
                取消
              </Button>
            </div>
          </Form.Item>
